...
# MAGIC %md
# MAGIC # Medallion Architecture: Gold Layer - Star Schema
# MAGIC 
# MAGIC **Purpose:** Build business-ready dimensional model from Silver layer
# MAGIC 
# MAGIC **Source:** Silver layer tables (cleaned, validated data)
# MAGIC 
# MAGIC **Target:** Gold layer - Star Schema for Sales Analytics
# MAGIC 
# MAGIC **Schema:**
# MAGIC - **Dimensions:** Dim_Date, Dim_Customer, Dim_Product, Dim_Address
# MAGIC - **Fact:** Fact_Sales
# MAGIC 
# MAGIC **Pattern:** Type 1 SCD with structure for Type 2 evolution
...
def get_silver_table(table_name):
    return f"{TARGET_LAKEHOUSE}.{SILVER_LAYER}_{table_name}"

def get_gold_table(table_name):
    return f"{GOLD_LAYER}_{table_name}"

logger.info("Gold layer configuration loaded")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 1. Build gold_Dim_Date

# COMMAND ----------

def generate_date_dimension(start_date='2000-01-01', end_date='2030-12-31'):
    """Generate comprehensive date dimension"""
    logger.info("Generating gold_Dim_Date...")
    ...
    dim_date = date_df.select(
        ...
        "DayOfWeek", ((dayofweek(col("Date")) + 5) % 7) + 1  # ISO 8601: Mon=1
    ).withColumn(
    ...
    logger.info(f"  Generated {row_count:,} date rows")
    
    return dim_date

dim_date = generate_date_dimension()
dim_date.write.format("delta").mode("overwrite").saveAsTable(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Date')}")
logger.info("✓ gold_Dim_Date created")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 2. Build gold_Dim_Customer

# COMMAND ----------

def build_gold_customer():
    """Build gold customer dimension from silver"""
    logger.info("Building gold_Dim_Customer...")
    
    silver_customer = spark.read.table(get_silver_table("Customer"))
    ...
    logger.info(f"  Built {row_count:,} customer dimension rows")
    
    return dim_customer

dim_customer = build_gold_customer()
dim_customer.write.format("delta").mode("overwrite").saveAsTable(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Customer')}")
logger.info("✓ gold_Dim_Customer created")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 3. Build gold_Dim_Product

# COMMAND ----------

def build_gold_product():
    """Build gold product dimension with denormalized hierarchy"""
    logger.info("Building gold_Dim_Product...")
    
    silver_product = spark.read.table(get_silver_table("Product"))
    silver_category = spark.read.table(get_silver_table("ProductCategory"))
    silver_model = spark.read.table(get_silver_table("ProductModel"))
    
    # Self-join category for parent
    parent_category = silver_category.select(
        col("ProductCategoryID").alias("ParentCategoryID"),
        col("Name").alias("ParentProductCategoryName")
    )
    
    category_with_parent = silver_category \
        .join(parent_category, 
              silver_category.ParentProductCategoryID == parent_category.ParentCategoryID,
              "left") \
        .select(
            silver_category.ProductCategoryID,
            col("Name").alias("ProductCategoryName"),
            silver_category.ParentProductCategoryID,
            "ParentProductCategoryName"
        )
    
    # Join Product → Category → Model
    dim_product = silver_product \
        .join(category_with_parent, "ProductCategoryID", "left") \
        .join(silver_model.select(col("ProductModelID"), col("Name").alias("ProductModelName")), 
              "ProductModelID", "left")
    
    # Add calculated fields
    dim_product = dim_product \
        .withColumn("IsDiscontinued", col("DiscontinuedDate").isNotNull()) \
        .withColumn("ProductName", lit(None).cast("string"))  # Will be enriched if description tables available
    ...
    logger.info(f"  Built {row_count:,} product dimension rows")
    
    return dim_product

dim_product = build_gold_product()
dim_product.write.format("delta").mode("overwrite").saveAsTable(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Product')}")
logger.info("✓ gold_Dim_Product created")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 4. Build gold_Dim_Address

# COMMAND ----------

def build_gold_address():
    """Build gold address dimension"""
    logger.info("Building gold_Dim_Address...")
    
    silver_address = spark.read.table(get_silver_table("Address"))
    
    dim_address = silver_address.select(
        col("AddressID"),
        col("AddressLine1"),
        col("AddressLine2"),
        col("City"),
    ...
    row_count = dim_address.count()
    logger.info(f"  Built {row_count:,} address dimension rows")
    
    return dim_address

dim_address = build_gold_address()
dim_address.write.format("delta").mode("overwrite").saveAsTable(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Address')}")
logger.info("✓ gold_Dim_Address created")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 5. Build gold_Fact_Sales

# COMMAND ----------

def build_gold_fact_sales():
    """Build gold sales fact table"""
    logger.info("Building gold_Fact_Sales...")
    
    # Read silver fact tables
    silver_detail = spark.read.table(get_silver_table("SalesOrderDetail"))
    silver_header = spark.read.table(get_silver_table("SalesOrderHeader"))
    silver_product = spark.read.table(get_silver_table("Product"))
    
    # Read gold dimensions for lookups
    dim_customer_lookup = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Customer')}")
    dim_product_lookup = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Product')}")
    dim_date_lookup = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Date')}")
    dim_address_lookup = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Address')}")
    
    # Join detail with header
    fact_sales = silver_detail.join(silver_header, "SalesOrderID")
    
    # Join with product for standard cost
    fact_sales = fact_sales.join(
        silver_product.select("ProductID", col("StandardCost").alias("ProductStandardCost")),
        "ProductID"
    )
    
    # Calculate measures (if not already in silver)
    fact_sales = fact_sales \
        .withColumn("LineTotal", col("LineTotal")) \
        .withColumn("LineProfit", col("LineTotal") - (col("OrderQty") * col("ProductStandardCost"))) \
        .withColumn("OrderTotal", col("TotalDue"))
    ...
    # Lookup BillToAddressKey (nullable)
    fact_sales = fact_sales.join(
    ...
    logger.info(f"  Built {row_count:,} fact rows")
    
    return fact_sales

fact_sales = build_gold_fact_sales()
fact_sales.write.format("delta").mode("overwrite").saveAsTable(f"{TARGET_LAKEHOUSE}.{get_gold_table('Fact_Sales')}")
logger.info("✓ gold_Fact_Sales created")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 6. Gold Layer Validation
...
gold_dim_customer = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Customer')}")
gold_dim_product = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Product')}")
gold_dim_date = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Date')}")
gold_dim_address = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Dim_Address')}")
gold_fact_sales = spark.read.table(f"{TARGET_LAKEHOUSE}.{get_gold_table('Fact_Sales')}")

logger.info(f"gold_Dim_Date: {gold_dim_date.count():,} rows")
logger.info(f"gold_Dim_Customer: {gold_dim_customer.count():,} rows")
logger.info(f"gold_Dim_Product: {gold_dim_product.count():,} rows")
logger.info(f"gold_Dim_Address: {gold_dim_address.count():,} rows")
...
# MAGIC %md
# MAGIC ## 7. Sample Analytics Queries
...
# MAGIC SELECT 
# MAGIC     d.Year,
# MAGIC     COUNT(DISTINCT f.SalesOrderID) as Orders,
# MAGIC     SUM(f.LineTotal) as Revenue,
# MAGIC     SUM(f.LineProfit) as Profit
# MAGIC FROM SalesAnalytics.gold_Fact_Sales f
# MAGIC JOIN SalesAnalytics.gold_Dim_Date d ON f.OrderDateKey = d.DateKey
